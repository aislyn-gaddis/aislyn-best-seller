---
title: "Bestsellers Analysis Part 2"
output:
  html_document:
    df_print: paged
---
...
- What authors have the highest appearances per book?
...
Colleen Hoover blows everyone else out of the water with 415 total appearances.
...
bestsellers_appearances |> # start with the data
   filter(total_appearances > 145) |> # only include rows with total_appearances over 400
  write_csv("data-processed/most-appearances.csv") |> # save the data for data wrapper
  print() # print it
```

## What authors have had the most appearances this year? last 2 years? 5 years? 10 years?

First, I'll find which authors have had the most appearances this year.

Again, Colleen Hoover blows everyone else out of the water with more than triple 2nd place. 

```{r most-appearances-this-year}
bestsellers |> # start with the data
  filter(year > 2022) |> # only include rows with a year over 2018
  group_by(author) |> # group by author
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 14) # only include appearances above 6
```

I'll look at the past two years.

Colleen Hoover again. More than triple second place.

```{r most-appearances-two-year}
bestsellers |> # start with the data
  filter(year > 2021) |> # only include rows with a year over 2018
  group_by(author) |> # group by author
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 22) # only include appearances above 6
```

I'll look at the past five years.

Colleen Hoover again, still with a large lead, but somewhat less than in the previous ones. 
...
bestsellers |> # start with the data
  ...
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 55) # only include appearances above 6
```

I'll look at 10 years.

This is crazy to me that Colleen Hoover still has the lead despite only becoming popular a few years ago. Note that she only has 6 appearances 2013-2017 and still takes the lead.

```{r most-appearances-ten-year}
bestsellers |> # start with the data
  filter(year > 2013) |> # only include rows with a year over 2018
  group_by(author) |> # group by author
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 105) # only include appearances above 6
```

## What authors have had the most No. 1 appearances this year? last 2 years? 5 years? 10 years?
...
bestsellers |> # start with the data
  filter(year > 2022,
         rank == 1) |> # only include rows with a year over 2018
  group_by(author) |> # group by author
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 1) # only include appearances above 6
```
...
bestsellers |> # start with the data
  ...
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 2) # only include appearances above 6
```

5 years?

First thing Colleen Hoover has not won lol. Delia Owens takes it most likely with Where the Crawdads Sing.
...
bestsellers |> # start with the data
  ...
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 5) # only include appearances above 6
```

And finally, 10 years?

Also Delia Owens with Where the Crawdads Sing, but closley followed by John Grisham.
...
bestsellers |> # start with the data
  ...
  summarize(appearances = n()) |> # count appearances per author
  arrange(desc(appearances)) |> # arrange in descending order by appearances
  filter(appearances > 10) # only include appearances above 6
```
...
bestsellers |> # start with the data
  filter(year >2022) |> # only include rows with a rank of 1 and a year over 2021
...
It's Danielle Steel with Colleen Hoover in a close second.

```{r most-books-two-years}
bestsellers |> # start with the data
  filter(year >2021) |> # only include rows with a rank of 1 and a year over 2021
  distinct(author, title) |> # only include unique author/title combinations
  group_by(author) |> # group by author
  summarize(titles = n()) |> # count titles per author
  arrange(desc(titles)) |> # arrange in descending order by titles
  filter(titles > 3) # only include titles above 1
```

I'll look at the last 5 years.

It's Danielle Steel again.

```{r most-books-last-five}
bestsellers |> # start with the data
  filter(year > 2018) |> # only include rows with a rank of 1 and a year over 2018
  distinct(author, title) |> # only include unique author/title combinations
  group_by(author) |> # group by author
  summarize(titles = n()) |> # count titles per author
  arrange(desc(titles)) |> # arrange in descending order by titles
  filter(titles > 9) # only include titles above 3
```

Now, I'll look at the last 10 years.

It's also Danielle Steel.

```{r most-ones-last-ten}
bestsellers |> # start with the data
  filter(year > 2013) |> # only include rows with a rank of 1 and a year over 2012
  distinct(author, title) |> # only include unique author/title combinations
  group_by(author) |> # group by author
  summarize(titles = n()) |> # count titles per author
  arrange(desc(titles)) |> # arrange in descending order by titles
  filter(titles > 17) # only include titles above 7
```
...
Delia Owens wins with 193 appearances for Where the Crawdads Sing. Colleen Hoover is the only person with 2 books in the top 10
...
bestsellers |> # start with the data
  ...
  filter(appearances > 67) |> # only include rows with appearances greater than 105
...
Where the Crawdads Sing was number one for the most number of weeks by a good amount.
...
bestsellers |> # start with the data
  ...
  summarize(appearances = n()) |> # count number of appearances
  arrange(desc(appearances)) |> # arrange in descending order
  filter(appearances > 6) |> # only include rows with appearances greater than 33
...
bestsellers |> # start with the data
  filter(str_detect(title, "WHERE THE CRAWDADS SING")) |> # only inlcude The Da Vinci Code
...
bestsellers |> # start with the data
  filter(str_detect(title, "FIFTY SHADES OF GREY")) |> # only inlcude The Da Vinci Code
  group_by(year) |> # group by year
  summarize(appearance = n()) # count the number of appearances in each year
```
...
```{r gone-girl}
bestsellers |> # start with the data
  filter(str_detect(title, "GONE GIRL")) |> # only inlcude The Da Vinci Code
  group_by(year) |> # group by year
  summarize(appearance = n()) # count the number of appearances in each year
```
...
bestsellers |> # start with the data
  ...
  arrange(desc(titles)) |> # arrange in descending order by titles
  filter(titles > 8) # only include rows with titles greater than 16
```
...
Colleen Hoover and John Sandford are tied for first.
...
bestsellers |> # start with the data
  filter(rank == 1,
         year > 2013) |> # only include rows with a rank of 1 and a year over 2012
  distinct(author, title) |> # only include unique author/title combinations
  group_by(author) |> # group by author
  summarize(titles = n()) |> # count titles per author
  arrange(desc(titles)) |> # arrange in descending order by titles
